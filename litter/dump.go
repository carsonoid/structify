...
var k8sPackageNameRegexp = regexp.MustCompile("apis?/[a-zA-Z_-]+/[a-zA-Z_-]+[0-9]+")
var packageNameStripperRegexp = regexp.MustCompile("\\b[a-zA-Z_]+[a-zA-Z_0-9]+\\.")
var compactTypeRegexp = regexp.MustCompile(`\s*([,;{}()])\s*`)
var varNameReplaceRegexp = regexp.MustCompile("![a-z1-9]")
...
type dumpState struct {
	...
	pointerAliases     map[string]interface{}
...
// Metadata holds additional information about the dump beyond the raw struct
...
func newMetadata() *Metadata {
	return &Metadata{
		...
		PointerAliases: make(map[string]interface{}),
	}
}
...
func (s *dumpState) dumpType(v reflect.Value) {
	...
	if k8sPackageNameRegexp.Match([]byte(pkgPath)) {
		pkgAlias := pkgParts[len(pkgParts)-2] + pkgParts[len(pkgParts)-1]
		s.packageAliases[pkgAlias] = pkgPath
		typeName = pkgAlias + "." + typeParts[1]
	}
...
func (s *dumpState) dumpPtrVal(v reflect.Value) {
	...
	case
		...
		ref := fmt.Sprintf("var_%v", v.Elem())
		ref = varNameReplaceRegexp.ReplaceAllString(ref, "")
		ref = strings.ToLower(ref)
		s.pointerAliases[ref] = v.Elem()
		s.w.Write([]byte("&"))
		s.w.Write([]byte(ref))
	default:
		s.w.Write([]byte("&"))
...
// Parse returns a string of the dump and the metadata for it
func (o Options) Parse(values ...interface{}) *ParseResult {
	buf := new(bytes.Buffer)
	md := newMetadata()
	for i, value := range values {
		if i > 0 {
		...
		state := newDumpState(value, &o, buf)
		state.dump(value)
		for k, v := range state.packageAliases {
			md.PackageAliases[k] = v
		}
		for k, v := range state.pointerAliases {
			md.PointerAliases[k] = v
		}
	}
	return &ParseResult{
		Dump:     buf.String(),
